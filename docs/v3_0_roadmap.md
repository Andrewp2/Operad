...
- Renderer-neutral testing helpers in `src/testing.rs` for event replay with
  ...
  paint-list assertions, RGBA pixel diffs with tolerances, dirty flags,
  deterministic snapshot hash/content assertions, frame timing section/budget
  assertions, command-aware shortcut replay assertions, and stable-name
  accessibility assertions for roles, labels, values, summaries, live regions,
  focus order, active descendants, document-frame generated platform-service
  requests, request/response coverage, correlated unsupported platform-service
  responses, and render-frame conformance assertions for canvas/image handler
  coverage, dirty regions, host input capture, and per-node interaction state.
