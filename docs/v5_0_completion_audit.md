...
| Compositor-quality rendering covers shadows, rounded clipping, borders, gradients, masks, filters, subpixel text, and parity for composited content. | Partial | `src/compositor.rs` records backend-neutral compositor-quality requirements and honest CPU/WGPU snapshot quality profiles; CPU and WGPU now both render `PaintBrush::LinearGradient` rich-rect fills, WGPU keeps SDF rounded-rect fills and glyphon text, and `tests/wgpu_snapshot_parity.rs` covers rich-rect gradient/effect pixels against CPU output. | Masks, filters, rounded clipping of composited child content, true blurred shadows, and subpixel-text parity are still not native visual-parity features; they remain explicit fallback/unsupported records until renderer composition passes exist. |
