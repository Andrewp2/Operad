...
| Text editing has undo/redo, robust IME lifecycle, multiline selection, and clipboard tests. | Partial | Widget text input state, IME host contracts, clipboard platform requests, `src/transactions.rs`. | Text input widgets do not yet consume `TextEditHistory` directly. |
...
| Font, icon, image, and texture lifecycle behavior is documented and tested. | Partial | `src/assets.rs`, `src/resource_cache.rs`, renderer resource descriptors, WGPU resource paths. | Font registry lifecycle is still being finalized. |
